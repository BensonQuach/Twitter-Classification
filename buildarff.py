...
adjusting the __str__ method, adding its respective function and adjusting
the schema.
'''
class FeatureCounter:
    ...
    def __init__(self, cls_name, tweet):
        ...
        if tweet == "":
            self.fpps = self.spps = self.tpps = self.ccs = self.ptvs = \
                self.ftvs = self.commas = self.all_colons = self.dashes = \
                self.parentheses = self.ellipses = self.cns = self.pns = \
                self.advs = self.wh_words = self.slangs = self.all_uppers = \
            ...
            self.ftvs = self.count_future_verbs()
            ...
            self.wh_words = self.count_wh_words()
    ...
    def __str__(self):
        ...
        (self.fpps, self.spps, self.tpps, self.ccs, self.ptvs, self.ftvs,
        self.commas, self.all_colons, self.dashes, self.parentheses,
        self.ellipses, self.cns, self.pns, self.advs, self.wh_words,
    ...
    def count_future_verbs(self):
        
        count = self.sum_by_list(FUTURE_TENSE, True)

        going = False
        to = False
        
        for word in self.lst_of_tokens:
            token = word[:word.rfind("/")].lower()

            if going and to and word.endswith("/VB"):
                count += 1
                going = False
                to = False
            elif going and not to and token == "to":
                to = True
            elif token == "going":
                going = True
            else:
    ...
    def count_wh_words(self):
        
        return self.sum_by_list(WH_WORDS, False)
